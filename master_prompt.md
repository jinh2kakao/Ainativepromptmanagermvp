...
-   **AI & Optimization**: 
    ...
    -   **Scoring**: Native Python evaluation service within Backend (The Judge).
    -   **Worker**: Asynchronous Python Worker (Native SQL Polling) for handling optimization jobs.
...
3.  `src/hooks/`:
    -   Global/Shared TanStack Query hooks.
    -   *Note*: Feature-specific hooks can be colocated in `src/features/{featureName}/` for better modularity.
...
## Recent Learnings & Guidelines (Updated: 2025-12-10)

### 1. Database & Templates
-   **Assistance Mode Structure**: 
    -   **Format**: Stored as **Markdown Text** (not JSON) using standard headers (e.g., `## Persona`, `## Instruction`).
    -   **Parsing**: The frontend uses a custom `pairParser` utility to extract sections via Regex and populate the structured UI. This replaces the legacy JSON format for better flexibility.
...
-   **Updates**:
    -   **Backend**: **GitHub Actions** (Push to `main` -> Auto Deploy). See `docs/guides/github_actions_setup.md`.
    -   **Frontend**: Push to `main` -> Cloudflare Pages auto-builds.

### 4. Team Collaboration (v3.0.0)
-   **Team Architecture**:
    -   `Team` & `TeamMember`: Models added to support multi-tenant workspaces.
    -   **RBAC**: Roles (Owner, Admin, Editor, Viewer) enforce robust permission checks.
    -   **Settings**: Dedicated `/teams/[id]/settings` page for member management.
    -   **API**: `PATCH` / `DELETE` endpoints for role updates and member removal.
-   **Future Phase (Locking)**: Project locking (Mutex) will prevent concurrent edit conflicts.

### 3. Supabase Auth for Static Hosting (CRITICAL)
-   **Client Library**: Use `@supabase/supabase-js` (`createClient`) instead of `@supabase/ssr` (`createBrowserClient`) for static exports (Cloudflare Pages, GitHub Pages).
-   **Reason**: `@supabase/ssr` uses cookie-based session management which can fail on static hosts due to CORS/SameSite policies. Standard client uses localStorage. DO NOT use `@supabase/ssr`.
-   **OAuth Configuration**:
    ```typescript
    ...
    ```
-   **Middleware Exclusion**: Always exclude `/auth/callback` from middleware matchers to prevent session check interference before code exchange.
-   **Error Handling**: Gracefully handle `Invalid Refresh Token` errors by calling `signOut()` and clearing session state.
...
-   **Guide Documents**: Store operational guides in `docs/guides/` for easy reference.
    -   `SSL_SETUP_GUIDE.md`: Cloudflare Origin Cert installation (Korean)
    -   `CORS_TROUBLESHOOTING.md`: CORS/522 error diagnosis
    -   `SSL_CORS_VICTORY_SUMMARY.md`: Problem resolution summary
