...
export function PromptModal({ prompt, onSave, onClose }: PromptModalProps) {
  ...
  const defaultStructure: NonNullable<Prompt['structure']> = {
    job: '',
  ...
  const [assistanceStructure, setAssistanceStructure] = useState<NonNullable<Prompt['structure']>>(
    (prompt?.mode === 'assistance' && prompt.structure)
      ? prompt.structure as NonNullable<Prompt['structure']>
      : defaultStructure
  );
  ...
  const [jobCategories, setJobCategories] = useState<CategoryOption[]>([]);
  ...
  const applyTemplate = (templateData: any) => {
    ...
    if (mode === 'simple') {
      setSimpleContent(templateData.content);
    } else {
      try {
        const parsed = JSON.parse(templateData.content);

        // Check if it's the new Group/Item structure (Array)
        if (Array.isArray(parsed)) {
          setTemplateSchema(parsed); // Store raw schema for dynamic rendering

          const newStructure: any = {
            job: subCategory || category, // Use selected category as job
            persona: {},
            asset: {},
            instruction: {},
            result: {}
          ...
          parsed.forEach((group: any) => {
            ...
            if (['persona', 'asset', 'instruction', 'result'].includes(groupName)) {
              group.items.forEach((item: any) => {
                let key = item.label.toLowerCase();
                if (key === 'knowledge base') key = 'knowledgeBase';
                if (key === 'style guide') key = 'styleGuide';

                // Initialize with template value
                newStructure[groupName][key] = item.value;
              });
            }
          });
          setAssistanceStructure(newStructure);
        } else {
          // Old P.A.I.R structure (Object)
          setAssistanceStructure(parsed);
          setTemplateSchema([]); // Reset schema if old format
        }
      } catch (e) {
        console.error('Failed to parse assistance template', e);
      }
    }

    if (templateData.applicable_agents) {
      setApplicableAgents(templateData.applicable_agents);
    }
  };

  // Fetch Default Templates
  useEffect(() => {
    // Only fetch if it's a NEW prompt (no existing prompt data being edited)
    if (prompt) return;

    const fetchTemplates = async () => {
      const targetValue = subCategory || category;
      if (!targetValue) {
        setTemplateSchema([]);
      ...
      const targetCat = flatCategories.find(c => c.value === targetValue);
      if (!targetCat) {
        console.warn('Target category not found for value:', targetValue);
        return;
      }

      setIsTemplateLoading(true);
      try {
        const url = `/api/categories/${targetCat.id}/template/?mode=${mode}`;

        const response = await api.get(url);
        const data = response.data;

        // Handle both array (new) and single object (old/fallback) response
        ...
        console.error('[QA] Failed to fetch templates', error);
        setTemplateSchema([]);
        setAvailableTemplates([]);
      } finally {
    ...
    fetchTemplates();
  }, [category, subCategory, mode, flatCategories]);

  // QA Log
  useEffect(() => {
    console.log('[QA] PromptModal availableTemplates:', availableTemplates);
    console.log('[QA] PromptModal selectedTemplateId:', selectedTemplateId);
  ...
  const handleAssistanceChange = (structure: NonNullable<Prompt['structure']>) => {
          ...
          <div className="space-y-4 md:space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="block text-sm text-gray-700">프롬프트 작성 모드</label>

                {/* Public/Private Toggle */}
                <button
                  onClick={() => setIsPublic(!isPublic)}
                  className={`flex items-center gap-1.5 px-2 py-1 rounded-md text-xs font-medium transition-colors ${isPublic
              ...
              <div className="flex flex-col md:flex-row md:items-center gap-3">
                <div id="tour-mode-switch" className="flex items-center gap-2 bg-gray-100 p-1 rounded-lg w-full md:w-fit shadow-inner">
                  <button
            ...
            {/* Applicable Agents Selection */}
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">권장 AI 모델 (선택)</label>
              <div className="flex flex-wrap gap-2">
                {AVAILABLE_AGENTS.map((agent) => {
                  const isSelected = applicableAgents.includes(agent.value);
                  return (
                    <button
                      key={agent.value}
                      onClick={() => {
                        const updated = isSelected
                          ? applicableAgents.filter(a => a !== agent.value)
                          : [...applicableAgents, agent.value];
                        setApplicableAgents(updated);
                      }}
