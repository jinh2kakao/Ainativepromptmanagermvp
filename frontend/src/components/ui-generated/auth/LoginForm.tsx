...
export function LoginForm({ onSwitchToSignUp, onLoginSuccess, onLogin, onGoogleLogin }: LoginFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Error is handled by AuthContainer (alert), but we can also set form error if needed
      console.error(error);
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="w-full max-w-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 md:space-y-5">
        ...
        <button
          ...
          className="w-full px-4 py-3 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-all flex items-center justify-center gap-2 md:gap-3 shadow-sm hover:shadow text-sm md:text-base min-h-[48px] disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isGoogleLoading ? (
          ...
          <span className="text-gray-700">{isGoogleLoading ? 'Google로 연결 중...' : 'Google로 계속하기'}</span>
        </button>
