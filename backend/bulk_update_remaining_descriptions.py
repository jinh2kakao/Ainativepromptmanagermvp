...
def update_all_descriptions():
    with Session(engine) as session:
        ...
        for template in templates:
            count += 1
            # If description is missing OR we want to update generic ones (optional, but let's stick to missing first to avoid overwriting good ones, or overwrite all?)
            # User said "ALL templates", so maybe I should overwrite if I'm confident.
            # But "전문적인..." fallback is weak. 
            # Strategy: If description is None or empty, strictly update.
            # If description exists but seems generated/weak (optional), maybe update.
            # Let's target missing ones primarily, and maybe "Default Template" types.
        ...
        session.commit()
        print(f"Processed {count} templates. Updated {updated_count} descriptions.")
