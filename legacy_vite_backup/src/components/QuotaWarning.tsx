...
export function QuotaWarning({ message, severity, onSignUp }: QuotaWarningProps) {
  ...
  return (
    <div
      className={`rounded-lg p-3 md:p-4 flex items-start gap-2 md:gap-3 shadow-sm ${
        severity === 'error'
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {onSignUp && (
          <button
            onClick={onSignUp}
            className={`mt-2 md:mt-3 px-3 md:px-4 py-2 rounded-lg text-xs md:text-sm transition-all duration-200 shadow-md hover:shadow-lg min-h-[40px] md:min-h-0 ${
              severity === 'error'
